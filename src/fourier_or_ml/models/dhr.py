...
class DynamicHarmonicRegression(Forecaster):
    def __init__(self, error_order: tuple[int, int, int] | None = (2, 0, 1)):
    ...
    def fit(self, y: pd.Series, X: pd.DataFrame | None = None) -> "DynamicHarmonicRegression":
        ...
        Xc = add_constant(X.to_numpy(dtype=float), has_constant="add")
    ...
    def predict(self, horizon: int, X_future: pd.DataFrame | None = None) -> pd.Series:
        ...
        Xf = add_constant(X_future.iloc[:horizon][self._cols].to_numpy(dtype=float),
                          has_constant="add")
